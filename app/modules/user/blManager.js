...
export default class BlManager {
  async updateUser(request) {
    try {
      ...
      if (request.profilePic) updateObj["profilePic"] = request.profilePic;
  ...
  signUp = async (requestData) => {
    ...
    return new UserSchema(requestData).save();
  };

  addUserCookies = async (requestData) => {
    return new UserCookiesSchema(requestData).save();
  };
  ...
  updateCookiesOfUser = async (requestData) => {
    Utils.lhtLog(
      "Auth0-srv: BL Manager",
      `update-user-cookies`,
      requestData,
      "",
      httpConstants.LOG_LEVEL_TYPE.INFO
    );
    return UserCookiesSchema.findAndUpdateData(
      { userId: requestData.userId },
      { cookiesAllowed: requestData.cookiesAllowed }
    );
  };

  privacyConsent = async (requestData) => {
    return UserCookiesSchema.findOneAndUpdateData(
      { userId: requestData.userId },
  ...
  globalIdUserInfo = async (requestData) => {
    try {
      ...
      let claims = await this.decryptClainTokens(
        requestData.idToken,
      ...
      if (!claims[0] || !claims[0].length)
        throw Utils.error(
          {},
          `Please verify your email id on GlobalId App`,
          httpConstants.RESPONSE_CODES.FORBIDDEN
        );
      claims = claims[0];
      ...
      if (!email)
        throw Utils.error(
          {},
          apiFailureMessage.NO_EMAIL_FOUND,
          httpConstants.RESPONSE_CODES.FORBIDDEN
        );

      const checkUser = await UserSchema.getUserDetails({
        email: email.value,
    ...
    try {
      ...
      claims.map((claim) => {
        if (claim.type && claim.value)
          responseData = {
            ...responseData,
            [globalIdConstants[claim.type]]: claim.value,
          };
      });
      responseData = {
        ...responseData,
  ...
  getGlobalIdTokens = async (requestData) => {
    ...
    let accessAndIdToken = JSON.parse(
      await this.accessAndIdToken(requestData.code)
    );
    if (
      !accessAndIdToken ||
      !accessAndIdToken.id_token ||
      !accessAndIdToken.access_token
    ) {
      throw Utils.error(
        {},
  ...
  async getTokensForThePII(id_token) {
    ...
    let consents = await _.filter(decoded_token, (value, index) => {
      return (
        _.includes(index, "idp.globalid.net/claims/") &&
    ...
    let response;
    try {
      response = _.flattenDeep(
        await Promise.all(consents.map(this.decryptedConsent))
      );
  ...
  async decryptClainTokens(idToken, accessToken) {
    ...
    const consents = filter(
      decoded_token,
      (value, index) =>
        index.includes("idp.globalid.net/claims/") && !isEmpty(value)
    );
    const result = [];
    for (let index = 0; index < consents.length; index++) {
      const res = await this.decryptPrivateDataTokenPerConsent(
        consents[index],
        accessToken
      );
      result.push(res);
  ...
  async decryptPrivateDataTokenPerConsent(consent, accessToken) {
    ...
    for (let index = 0; index < result.length; index++) {
      let password = gidCrypto.RSA.decrypt(
        Config.PRIVATE_KEY,
        result[index].encrypted_data_password
      );
      const tokenData = gidCrypto.AES.decrypt(
        result[index].encrypted_data,
        password
      );
